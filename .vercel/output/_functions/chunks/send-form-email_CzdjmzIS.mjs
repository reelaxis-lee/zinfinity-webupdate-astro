...
const POST = async ({ request }) => {
  ...
  const subject = isContact ? `Contact Form: ${name}${company ? ` at ${company}` : ""}` : `AI Readiness Assessment Request: ${name}${company ? ` at ${company}` : ""}`;
  const htmlBody = isContact ? `<h2>New Contact Form Submission</h2>
       ...
       </table>` : `<h2>New AI Readiness Assessment Request</h2>
       <table style="border-collapse:collapse;width:100%;max-width:600px;">
         <tr><td style="padding:8px;font-weight:bold;">Name</td><td style="padding:8px;">${escapeHtml(name)}</td></tr>
  ...
  try {
    ...
    const message2 = error instanceof Error ? error.message : "Failed to send email";
